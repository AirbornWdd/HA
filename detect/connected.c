...
connected_withdraw (struct connected *ifc)
{
  ...
  if (CHECK_FLAG (ifc->conf, KROUTE_IFC_REAL))
    {
      kroute_interface_address_delete_update (ifc->ifp, ifc);
...
connected_implicit_withdraw (struct interface *ifp, struct connected *ifc)
{
  ...
  if ((current = connected_check (ifp, (struct prefix *) ifc->address)))
    {
      if (CHECK_FLAG(current->conf, KROUTE_IFC_CONFIGURED))
       ...
       */
      if (connected_same (current, ifc) && CHECK_FLAG(current->conf, KROUTE_IFC_REAL))
        {
          /* nothing to do */
          connected_free (ifc);
          return NULL;
      ...
      UNSET_FLAG(current->conf, KROUTE_IFC_CONFIGURED);
		    ...
		    const char *label)
{
  ...
  if (broad)
    ...
    {
      if (CHECK_FLAG(ifc->flags, KROUTE_IFA_PEER))
        {
	  zlog_warn("warning: %s called for interface %s "
		    "with peer flag set, but no peer address supplied",
		    __func__, ifp->name);
	  UNSET_FLAG(ifc->flags, KROUTE_IFA_PEER);
	}
		    ...
		    const char *label)
{
  ...
  if (CHECK_FLAG(ifc->flags, KROUTE_IFA_PEER) && !ifc->destination)
    {
      zlog_warn("warning: %s called for interface %s "
		"with peer flag set, but no peer address supplied",
		__func__, ifp->name);
