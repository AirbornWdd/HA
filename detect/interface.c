...
static int
ha_if_new_hook (struct interface *ifp)
{
  struct ha_if_info *ha_if;

  ha_if = XCALLOC (MTYPE_HA_IF_INFO, sizeof (struct ha_if_info));

  ha_if->shutdown = IF_KROUTE_SHUTDOWN_UNSPEC;

  /* Initialize installed address chains tree. */
  ha_if->ipv4_subnets = route_table_init ();
...
static int
ha_if_delete_hook (struct interface *ifp)
{
  struct ha_if_info *ha_if;
  ...
  if (ifp->info)
    {
      ...
      if (ha_if->ipv4_subnets)
	route_table_finish (ha_if->ipv4_subnets);

      XFREE (MTYPE_HA_IF_INFO, ha_if);
    }
...
if_subnet_add (struct interface *ifp, struct connected *ifc)
{
  ...
  cp = *ifc->address;
  apply_mask (&cp);
  rn = route_node_get (ha_if->ipv4_subnets, &cp);

  if ((addr_list = rn->info))
    SET_FLAG (ifc->flags, KROUTE_IFA_SECONDARY);
  else
    {
      UNSET_FLAG (ifc->flags, KROUTE_IFA_SECONDARY);
...
if_subnet_delete (struct interface *ifp, struct connected *ifc)
{
  ...
  rn = route_node_lookup (ha_if->ipv4_subnets, ifc->address);
  if (! (rn && rn->info))
    return -1;
  ...
  if (addr_list->count)
      ...
      if (! CHECK_FLAG (ifc->flags, KROUTE_IFA_SECONDARY))
	{
	  ...
	  kroute_interface_address_delete_update (ifp, ifc);
	  UNSET_FLAG (ifc->flags, KROUTE_IFA_SECONDARY);
	  kroute_interface_address_add_update (ifp, ifc);
	}
 ...
 * struct ha_if_info.primary_state.
 *
 * Setting IFF_UP within kroute to administratively shutdown the
...
{
#ifdef SUNOS_5
  struct ha_if_info *zif = ifp->info;
  ...
  for (ALL_LIST_ELEMENTS (ifp->connected, node, nnode, ifc))
    {
      ...
      if (CHECK_FLAG (ifc->conf, KROUTE_IFC_CONFIGURED)
	  && ! CHECK_FLAG (ifc->conf, KROUTE_IFC_REAL))
	{
	  /* Address check. */
	  if (p->family == AF_INET)
	    {
	      ...
	      if (ret < 0)
		{
	      ...
	      SET_FLAG (ifc->conf, KROUTE_IFC_REAL);
...
{
  struct ha_if_info *if_data;

  if_data = ifp->info;

  kroute_interface_add_update (ifp);

  if (! CHECK_FLAG (ifp->status, KROUTE_INTERFACE_ACTIVE))
    {
      SET_FLAG (ifp->status, KROUTE_INTERFACE_ACTIVE);
...
if_delete_update (struct interface *ifp)
{
  ...
  UNSET_FLAG (ifp->status, KROUTE_INTERFACE_ACTIVE);
  ...
  if (ifp->connected)
      ...
      while ((node = (last ? last->next : listhead (ifp->connected))))
	{
	  ...
	  if (p->family == AF_INET
	      && (rn = route_node_lookup (ha_if->ipv4_subnets, p)))
	    {
	      ...
	      for (anode = first->next; anode || first; anode = next)
		{
		  ...
		  UNSET_FLAG (ifc->conf, KROUTE_IFC_REAL);
  ...
  kroute_interface_delete_update (ifp);
...
if_down (struct interface *ifp)
{
  ...
  kroute_interface_down_update (ifp);
...
connected_dump_vty (struct vty *vty, struct connected *connected)
{
  ...
  if (CHECK_FLAG (connected->flags, KROUTE_IFA_SECONDARY))
...
if_dump_vty (struct vty *vty, struct interface *ifp)
{
  ...
  else if (! CHECK_FLAG (ifp->status, KROUTE_INTERFACE_ACTIVE))
    {
  ...
  for (rn = route_top (ha_if->ipv4_subnets); rn; rn = route_next (rn))
    {
      if (! rn->info)
	continue;
  ...
  for (ALL_LIST_ELEMENTS_RO (ifp->connected, node, connected))
    {
      if (CHECK_FLAG (connected->conf, KROUTE_IFC_REAL) &&
...
/* Wrapper hook point for kroute daemon so that ifindex can be set 
 * DEFUN macro not used as extract.pl HAS to ignore this
 * See also interface_cmd in lib/if.c
 */ 
DEFUN_NOSH (kroute_interface,
	    kroute_interface_cmd,
	    "interface IFNAME",
       ...
       "Enable link detection on interface\n")
{
  ...
  SET_FLAG(ifp->status, KROUTE_INTERFACE_LINKDETECTION);
       ...
       "Disable link detection on interface\n")
{
  ...
  ifp = (struct interface *) vty->index;
  if_was_operative = if_is_operative(ifp);
  UNSET_FLAG(ifp->status, KROUTE_INTERFACE_LINKDETECTION);
       ...
       "Shutdown the selected interface\n")
{
  ...
  if_refresh (ifp);
  if_data = ifp->info;
  if_data->shutdown = IF_KROUTE_SHUTDOWN_ON;
       ...
       "Shutdown the selected interface\n")
{
  ...
  if_refresh (ifp);
  if_data = ifp->info;
  if_data->shutdown = IF_KROUTE_SHUTDOWN_OFF;
		    ...
		    const char *label)
{
  ...
  /* This address is configured from kroute. */
  if (! CHECK_FLAG (ifc->conf, KROUTE_IFC_CONFIGURED))
    SET_FLAG (ifc->conf, KROUTE_IFC_CONFIGURED);

  /* In case of this route need to install kernel. */
  if (! CHECK_FLAG (ifc->conf, KROUTE_IFC_REAL)
      && CHECK_FLAG (ifp->status, KROUTE_INTERFACE_ACTIVE))
    {
		      ...
		      const char *label)
{
  ...
  UNSET_FLAG (ifc->conf, KROUTE_IFC_CONFIGURED);
...
if_config_write (struct vty *vty)
{
  ...
  for (ALL_LIST_ELEMENTS_RO (iflist, node, ifp))
	...
	vty_out(vty, " link-detect%s", VTY_NEWLINE);
      ...
      if (if_data)
	{
	  if (if_data->shutdown == IF_KROUTE_SHUTDOWN_ON)
...
ha_if_init (void)
{
  /* Initialize Kroute interface data structure. */
  if_init ();
  hm->iflist = iflist;
  if_add_hook (IF_NEW_HOOK, ha_if_new_hook);
  if_add_hook (IF_DELETE_HOOK, ha_if_delete_hook);
  ...
  install_element (CONFIG_NODE, &kroute_interface_cmd);
